...
namespace Miningcore.Api.Controllers
{
    ...
    {
        static PledgeContract contract;
        ...
        [HttpGet("{poolId}/depositpower")]
        public async Task<string> GetDepositPowerAsync(string address)
        {
            /*if(contract == null)
            {
                contract = new PledgeContract(clusterConfig.OdinEndpoint);
            }
            BigInteger power = contract.GetDepositPower(address);
            BigInteger rigNumber = contract.GetOwnerRigNumber(address);

            string result = "地址" + address + " 抵押总算力:" + power.ToString() + "，已绑定矿机数量:" + rigNumber.ToString();
            
            return result;*/
            return "";
        }

        [HttpGet("{poolId}/deposit")]
        public async Task<string> GetPoolDepositAsync()
        {
            string depositValue = "";
/*            if(PledgeContract.deposits.Any())
            {
                foreach(var keyValuePair in PledgeContract.deposits)
                {
                    depositValue = depositValue + keyValuePair.Key + ":" + keyValuePair.Value + ";";
